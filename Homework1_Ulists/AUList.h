...
public:
  ...
  int getMin(); //Returns the minimum possible value for list elements.

  int getRange(); //Returns the range (max-min) of list elements.

  AUList DuplicateSE(int first, int last); //Returns a new AUList object that is a duplicate of the current list, but only including elements from index 'first' to index 'last' (inclusive). Assumes 0-based indexing.
